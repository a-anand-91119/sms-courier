...
object CommandPatterns {
    // Master pattern to detect any SMSC command
    val SMSC_COMMAND = Regex(
        """SMSC\s+(PAIR_REQUEST|PAIR_APPROVED|PAIR_REJECTED|UNPAIR|AUTH_REQUEST|""" +
    ...
    val AUTH_CHALLENGE = Regex(
        """SMSC\s+AUTH_CHALLENGE\s+(\S+)\s*$""",
        RegexOption.IGNORE_CASE,
    )

    val PAIR_APPROVED = Regex(
        """SMSC\s+PAIR_APPROVED\s*$""",
        RegexOption.IGNORE_CASE,
    )

    val PAIR_REJECTED = Regex(
        """SMSC\s+PAIR_REJECTED\s*$""",
        RegexOption.IGNORE_CASE,
    )
    ...
    val START_FORWARD = Regex(
        """SMSC\s+START_FORWARD\s+(\S+)(?:\s+(\d+))?\s*$""",
        RegexOption.IGNORE_CASE,
    )

    val STOP_FORWARD = Regex(
        """SMSC\s+STOP_FORWARD\s*$""",
        RegexOption.IGNORE_CASE,
    )
    ...
    // SMSC start <password> or SMSC stop <password>
    // Group 1: start/stop, Group 2: password
    val LEGACY_START_STOP = Regex(
        """SMSC\s+(start|stop)\s+(\S+)""",
        RegexOption.IGNORE_CASE,
    )

    fun isSmscCommand(message: String): Boolean {
        return message.trim().startsWith("SMSC", ignoreCase = true)
    }
}
