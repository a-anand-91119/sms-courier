...
class CommandParserTest {
    ...
    @Test
    fun `parse returns null for non-OTPC message`() {
    ...
    @Test
    fun `parse returns null for message starting with OTP but not OTPC`() {
    ...
    fun `parse PAIR_REQUEST returns PairRequest`() {
        val result = CommandParser.parse(testPhoneNumber, "OTPC PAIR_REQUEST")
    ...
    fun `parse PAIR_APPROVED returns PairApproved`() {
        val result = CommandParser.parse(testPhoneNumber, "OTPC PAIR_APPROVED")
    ...
    fun `parse PAIR_REJECTED returns PairRejected`() {
        val result = CommandParser.parse(testPhoneNumber, "OTPC PAIR_REJECTED")
    ...
    fun `parse UNPAIR returns Unpair`() {
        val result = CommandParser.parse(testPhoneNumber, "OTPC UNPAIR")
    ...
    fun `parse AUTH_REQUEST returns AuthRequest`() {
        val result = CommandParser.parse(testPhoneNumber, "OTPC AUTH_REQUEST")
    ...
    fun `parse AUTH_CHALLENGE extracts nonce`() {
        val result = CommandParser.parse(testPhoneNumber, "OTPC AUTH_CHALLENGE abc123nonce")
    ...
    fun `parse AUTH_CHALLENGE with base64 nonce`() {
        ...
        val result = CommandParser.parse(testPhoneNumber, "OTPC AUTH_CHALLENGE $base64Nonce")
        assertTrue(result is ParsedCommand.AuthChallenge)
    ...
    fun `parse AUTH_CHALLENGE case insensitive`() {
        val result = CommandParser.parse(testPhoneNumber, "otpc auth_challenge testNonce")
        assertTrue(result is ParsedCommand.AuthChallenge)
    ...
    fun `parse START_FORWARD with password only`() {
        val result = CommandParser.parse(testPhoneNumber, "OTPC START_FORWARD myPassword123")
    ...
    fun `parse START_FORWARD with password and duration`() {
        val result = CommandParser.parse(testPhoneNumber, "OTPC START_FORWARD myPassword123 60")
        assertTrue(result is ParsedCommand.StartForward)
        val cmd = result as ParsedCommand.StartForward
        assertEquals("myPassword123", cmd.password)
    ...
    fun `parse START_FORWARD with HMAC response format`() {
        ...
        val result = CommandParser.parse(testPhoneNumber, "OTPC START_FORWARD $hmacResponse 30")
        assertTrue(result is ParsedCommand.StartForward)
        val cmd = result as ParsedCommand.StartForward
    ...
    fun `parse START_FORWARD case insensitive`() {
        val result = CommandParser.parse(testPhoneNumber, "otpc start_forward password123")
        assertTrue(result is ParsedCommand.StartForward)
    }
    ...
    fun `parse STOP_FORWARD returns StopForward`() {
        val result = CommandParser.parse(testPhoneNumber, "OTPC STOP_FORWARD")
    ...
    fun `parse FWD extracts original sender and message`() {
        val result = CommandParser.parse(testPhoneNumber, "OTPC FWD <+9876543210> Your OTP is 123456")
    ...
    fun `parse FWD with short phone number`() {
        val result = CommandParser.parse(testPhoneNumber, "OTPC FWD <12345> Short code message")
        assertTrue(result is ParsedCommand.ForwardedData)
        val cmd = result as ParsedCommand.ForwardedData
    ...
    fun `parse FWD with message containing special characters`() {
        val result = CommandParser.parse(testPhoneNumber, "OTPC FWD <+1111111111> Code: 123-456, expires in 5 min!")
        assertTrue(result is ParsedCommand.ForwardedData)
        val cmd = result as ParsedCommand.ForwardedData
    ...
    fun `parse FWD case insensitive`() {
        val result = CommandParser.parse(testPhoneNumber, "otpc fwd <+5555555555> Test message")
        assertTrue(result is ParsedCommand.ForwardedData)
    }
    ...
    fun `parse FWDE extracts encrypted content`() {
        ...
        val result = CommandParser.parse(testPhoneNumber, "OTPC FWDE $encryptedContent")
    ...
    fun `parse FWDE with long encrypted content`() {
        ...
        val result = CommandParser.parse(testPhoneNumber, "OTPC FWDE $longContent")
        assertTrue(result is ParsedCommand.ForwardedDataEncrypted)
    ...
    fun `parse FWDE case insensitive`() {
        val result = CommandParser.parse(testPhoneNumber, "otpc fwde encryptedData123")
        assertTrue(result is ParsedCommand.ForwardedDataEncrypted)
    }
    ...
    fun `parse legacy start command`() {
        val result = CommandParser.parse(testPhoneNumber, "OTPC start secretPassword")
    ...
    fun `parse legacy stop command`() {
        val result = CommandParser.parse(testPhoneNumber, "OTPC stop secretPassword")
    ...
    fun `parse legacy command case insensitive`() {
        val result = CommandParser.parse(testPhoneNumber, "OTPC START password123")
        assertTrue(result is ParsedCommand.LegacyStart)
    }
    ...
    @Test
    fun `parse unknown OTPC command returns Unknown`() {
        val result = CommandParser.parse(testPhoneNumber, "OTPC INVALID_COMMAND")
        assertTrue(result is ParsedCommand.Unknown)
        assertEquals("OTPC INVALID_COMMAND", (result as ParsedCommand.Unknown).rawMessage)
    }

    @Test
    fun `parse malformed OTPC command returns Unknown`() {
        val result = CommandParser.parse(testPhoneNumber, "OTPC")
        assertTrue(result is ParsedCommand.Unknown)
    }
    ...
    @Test
    fun `isCommand returns true for OTPC prefix`() {
        assertTrue(CommandParser.isCommand("OTPC PAIR_REQUEST"))
    }

    @Test
    fun `isCommand returns false for non-OTPC message`() {
        assertFalse(CommandParser.isCommand("Hello World"))
    }

    @Test
    fun `isCommand is case insensitive`() {
        assertTrue(CommandParser.isCommand("otpc something"))
    }
